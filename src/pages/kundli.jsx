  ...
  return (
    <>
      <Header 
        ...
        hideEditIcon={true}
      />
      <img style={{width: '80%', height: 'auto', margin: '32px auto'}} src={kundaliBanner} alt="Kundali Banner" className="w-full h-40 object-cover" />
      <div className="container mx-auto px-4 mt-4 text-center font-eng">
        {initialLoading ? (
          <div className="flex justify-center items-center h-64">
            <div className="text-[#9A283D] text-lg">Loading kundli information...</div>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 theme_text max-w-md mx-auto">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#9A283D] border-2 border-[#9A283D] text-white py-3 rounded-xl shadow-lg font-medium text-lg disabled:opacity-50 hover:bg-[#7A1C2B] transition-colors mt-12"
                style={{marginTop: '40px', width: '80%'}}
              >
                {loading ? "Saving..." : "Yes I'm in"}
              </button>
               <button
                // type="submit"
                disabled={loading}
                onClick={() => navigate("/")}
                className="w-full bg-[white] border-2 border-[#9A283D] text-[#9A283D] py-3 rounded-xl shadow-lg font-medium text-lg disabled:opacity-50 hover:bg-[#7A1C2B] transition-colors mt-12"
