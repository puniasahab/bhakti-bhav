...
export default function PaymentComplete() {
  ...
  const baseParams = useGA4BaseParams("Payment Complete Screen");
      ...
      <div className="min-h-[calc(100vh-80px)] flex flex-col items-center justify-center px-5 py-10">
        ...
        <div className="w-full max-w-sm bg-[rgba(255,250,244,0.88)] backdrop-blur-sm rounded-3xl shadow-xl px-6 py-8 flex flex-col items-center">
          ...
          <div className="relative flex items-center justify-center mb-8">
            {/* Outer pulsing ring */}
            <span
              className={`absolute rounded-full transition-all duration-700 ease-out ${isSuccess ? "bg-[#9A283D]/10" : "bg-gray-300/40"
                } ${ringVisible ? "w-40 h-40 opacity-100" : "w-10 h-10 opacity-0"}`}
            />
            {/* Middle ring */}
            <span
              className={`absolute rounded-full transition-all duration-700 ease-out delay-100 ${isSuccess ? "bg-[#9A283D]/20" : "bg-gray-300/60"
                } ${ringVisible ? "w-28 h-28 opacity-100" : "w-6 h-6 opacity-0"}`}
            />
            {/* Inner circle with icon */}
            <div
              className={`relative z-10 w-20 h-20 rounded-full flex items-center justify-center shadow-lg transition-all duration-500 ease-out ${isSuccess ? "bg-[#9A283D]" : "bg-gray-400"
          ...
          {/* Text content */}
          <div
            className={`text-center transition-all duration-700 ease-out ${contentVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
              }`}
          >
            {isSuccess ? (
              <>
